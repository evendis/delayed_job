...
describe Delayed::Command do
  ...
  describe 'running worker pools defined by multiple --pool arguments' do
    it 'should run the correct worker processes' do
      ...
      [
        ['delayed_job_num0', {:quiet => true, :pid_dir => './tmp/pids', :log_dir => './log', :queues => []}],
        ['delayed_job_num1', {:quiet => true, :pid_dir => './tmp/pids', :log_dir => './log', :queues => ['test_queue']}],
        ['delayed_job_num2', {:quiet => true, :pid_dir => './tmp/pids', :log_dir => './log', :queues => ['test_queue']}],
        ['delayed_job_num3', {:quiet => true, :pid_dir => './tmp/pids', :log_dir => './log', :queues => ['test_queue']}],
        ['delayed_job_num4', {:quiet => true, :pid_dir => './tmp/pids', :log_dir => './log', :queues => ['test_queue']}],
        ['delayed_job_num5', {:quiet => true, :pid_dir => './tmp/pids', :log_dir => './log', :queues => %w[mailers misc]}],
        ['delayed_job_num6', {:quiet => true, :pid_dir => './tmp/pids', :log_dir => './log', :queues => %w[mailers misc]}]
      ].each do |args|
